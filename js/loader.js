...
function setConfigs(){
	...
	if(localStorage.getItem(config.site + 'configs') == null){
		...
		config.addlog = ['onload', 'loadRepos', 'Exception', 'all', 'getUpdatedWI', 'allRequest'];
		...
		config.repoitems = ['loadToken', 'loadCommit', 'getUpdatedWI', 'addWIs', 'processComment', 'loadComment', 'uploadImage', 'getRelaysWiql'];
		config.repo = "";
		config.key = "";
		config.device = "";
		config.maxDate = "";
...
function all(url, json, callBack, item, online, get) {	
	...
	log('all', 'allstart', "Opening DB: " + item + id, ln());
	request.onsuccess = function(event) {
		try {
			id = id == undefined ? 0 : id;
			log('all', 'onsuccess', "Getting DB: " + item + id, ln());
			var request = event.target.result.transaction("workitems", "readwrite").objectStore("workitems").get(Number(id));
			log('all', 'onsuccess', "Got DB: " + item + id, ln());
	
			request.onsuccess = function(e) {
			log('all', 'onsuccess2', "ProcessID: " + item + id, ln());
				if(id > 0){
					if(e.target.result != undefined){
						if(id != config.maxId){
							removeWI(id);
						}
						log('all', 'onsuccess2', "Removed ID: " + item + id, ln());
...
function allRequest(url, json, callBack, item, online, get, base ){
	...
	log('allRequest', 'starting', "Getting: " + item + url + "--" + base + online[0] + get, ln());
	$.ajax({
		url: destUrl,
		dataType: destUrl.includes('images/') ? 'json' : null,
		type : method,
		data: finalData,
		async: true,
		...
		success: function (str,sta,xhr) {
			if(xhr.status == 200){
				log('allRequest', 'success', "Calling handler: " + item + url + "--" + base + online[0] + get, ln());
		...
		failure: function (str,sta,xhr) {
			log('allRequest', 'onfailure', "Not found " + url + item, ln());
			online.shift();
			all(url, json, callBack, item, online, get);
		},
		error: function (str,sta,xhr) {
			log('allRequest', 'Error1', "Not found " + url + item, ln());
				...
				for(var i=json.commits[0].changes.length-1; i>=0; i--){
					if(json.commits[0].changes[i].changeType == 'add' && json.commits[0].changes[i].item.path == filename){
						...
						for(var j=objs.length - 1; j>=0; j--){
							var f = objs[j].filename;
							if("/images/" + objs[j].filename == filename){
								objs.splice(j, 1);
							}
						}
						edited = true;
			...
			log('allRequest', 'Error3', "Done." + item, ln());
		},
	});
}
...
function getMaxItem(context) {
	var result = getResult(context);
	config.maxId = result.id;
	config.maxDate = result.fields['System.ChangedDate'];
}
...
function log(funct, item, text, ln){
	...
	*/
	if(consolelog){
		...
		if(diff > 10){
			console.log( diff + "-" + ln + "-" + funct + ":" + item + ":" + text );
		}
...
function getUpdatedWI(context){	
	log('getUpdatedWI', 'start', "Getting Updated Work Items", ln());
	var result = getResult(context);	
	var ids = [];	
	for (var i in result.workItems) {
		ids.push(parseInt(result.workItems[i].id));
	}
	log('getUpdatedWI', 'ids', "Getting Updated Work Items: " + ids, ln());
	if(ids.length > 0){
...
function getLastDate(){
	log('getLastDate', 'start', "Getting last date", ln());	
	var request = window.indexedDB.open(config.site + "Database");
	request.onsuccess = function(event) {
		try{
			...
			log('getLastDate', 'onsuccess', "From DB", ln());	
			request.onsuccess = function() {
				...
				log('getLastDate', 'end', "Getting Updated Work Items", ln());
				var json = JSON.stringify({
	...
	try{
		switch (item) {
				...
				case 'uploadImage': url = config.url_org + '/' + config.path_push; 
								...
								ispost = true;
								break;
				case 'getMaxItem': url = config.fileserver + "images/maxItem.js"; 
								callBack = getMaxItem;
								ispost = false;
								online = ['sitecache'];
								break;
